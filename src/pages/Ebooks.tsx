  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-20">
      ...
      <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-6">
        {content.ebooks.filter((e: any) => e.status !== 'inactive').map((ebook: any) => (
          <div key={ebook.id} className="border rounded-xl overflow-hidden hover:shadow-lg transition-shadow relative">
            {ebook.tag && <div className="absolute top-2 right-2 bg-blue-600 text-white text-xs font-bold px-2 py-1 rounded z-10">{ebook.tag}</div>}
            {ebook.coverUrl ? (
